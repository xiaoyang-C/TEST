          ...
          <template v-else>
            <template v-if="typeof msg.content === 'object' && msg.content.analysis">
              ...
              <div class="coordinates-text">
                坐标: ({{ msg.content.coordinates.x1 }}, {{ msg.content.coordinates.y1 }}) 至 ({{ msg.content.coordinates.x2 }}, {{ msg.content.coordinates.y2 }})
              </div>
      ...
      reader.onload = (e) => {
        ...
        const imageMessage = {
          ...
          content: [
            {
              type: "text",
              text: "请分析这张图片中的物体，并进行目标识别。如果图像中有物体，请返回物体名称和异常位置的坐标（格式：{\"x1\": 左上角x, \"y1\": 左上角y, \"x2\": 右下角x, \"y2\": 右下角y}）。例如：{\"analysis\": \"钥匙\",\"coordinates\": {\"x1\": 100, \"y1\": 200, \"x2\": 150, \"y2\": 250}}"
            },
            {
              type: "image_url",
              image_url: {
